...
from motion_engine.studio.undo import SelectSessionCommand, SelectSubjectCommand, StudioUndoStack
...
class StudioController(QObject):
    ...
    def __init__(
        ...
        self.state = None
        self.events = None
        self.undo_stack: StudioUndoStack | None = None
        self._applying_undo = False
        self.tasks = TaskManager(self)
        self._busy = False
        self._timer = QTimer(self)
        self._timer.setInterval(16)  # ~60 Hz UI/playback clock
    ...
    def open_default_dataset(self) -> None:
        """Open the default / configured dataset (background TaskManager)."""
        self.open_dataset(None)
    def open_dataset(self, path: str | None) -> None:
        """Open a dataset path (``None`` = default) without blocking the UI."""
        if self._busy:
            return
        self._busy = True
            ...
            try:
                ...
                self.view.set_inspector_dataset(
                    self.analytics.dataset_info(
                        ...
                        dataset_path=str(model.dataset_path)
                        if model.dataset_path
                        else None,
                    )
                )
                self._sync_state_project(model)
                self._refresh_status()
            finally:
                self._busy = False
        ...
        def on_error(exc: Exception) -> None:
            ...
            msg = str(exc) if isinstance(exc, MotionServiceError) else str(exc)
            self.view.show_error("Dataset Error", msg)
        self.tasks.submit(
            work,
            on_success=on_success,
            on_error=on_error,
            on_progress=lambda value: self.view.show_loading_progress(int(value)),
            description="open_dataset",
        )
    # ------------------------------------------------------------- selection
    def select_subject(self, subject_id: str) -> None:
        """Load sessions for ``subject_id``."""
        if self.undo_stack is not None and not self._applying_undo:
            previous = self._active_subject
            def apply_subject(sid: str | None) -> None:
                self._applying_undo = True
                try:
                    if sid:
                        self._do_select_subject(sid)
                finally:
                    self._applying_undo = False
            ...
            return
        self._do_select_subject(subject_id)
    def _do_select_subject(self, subject_id: str) -> None:
        try:
    ...
    def select_session(self, session_name: str) -> None:
        """Build skeleton/clip for the active subject session (background)."""
        if not self._active_subject or self._busy:
            return
        if self.undo_stack is not None and not self._applying_undo:
            previous = self._active_session.name if self._active_session else None
            def apply_session(name: str | None) -> None:
                self._applying_undo = True
                try:
                    if name:
                        self._do_select_session(name)
                finally:
                    self._applying_undo = False
            self.undo_stack.push(
            ...
            return
        self._do_select_session(session_name)
    def _do_select_session(self, session_name: str) -> None:
        if not self._active_subject or self._busy:
            return
        ...
        def on_success(result) -> None:
            try:
                ...
                self.settings.remember_session(subject_id, session_name)
                self.view.set_recent_sessions(self.settings.recent_sessions)
                ...
                if self.events is not None:
                    self.events.emit_session_loaded(subject_id, session_name)
                self._refresh_status()
            finally:
                self._busy = False
                self.view.show_loading(None)
    ...
    def _sync_playback_ui(self) -> None:
        ...
        if self.state is not None:
            self.state.set_playback(
                playback_state=model.state,
                frame=model.current_frame,
                frame_count=model.frame_count,
                fps=model.fps,
                playing=model.state.value == "playing",
            )
        if self.events is not None:
            self.events.emit_playback_changed(model.state.value)
            self.events.emit_frame_changed(model.current_frame)
        self._refresh_status()
    def _sync_state_project(self, model) -> None:
        if self.state is None:
            return
        path = str(model.dataset_path) if model.dataset_path else None
        self.state.set_project(project_path=path)
        if self.events is not None and path:
            self.events.emit_dataset_loaded(path)
